...
export default function SubmissionsPage() {
  ...
  // 1. Cargar las Submissions (Solo el dueño de la encuesta puede verlas)
  useEffect(() => {
    const subscription = client.models.Submission.observeQuery().subscribe({
      ...
      error: (err) => {
        console.error("Error cargando submissions (¿Permisos?):", err);
        setLoading(false);
      },
    });
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {!loading && submissions.length === 0 && (
          <p className="text-center text-gray-500 py-10 border border-dashed rounded-lg bg-white">
            Aún no hay respuestas guardadas en tu base de datos.
          </p>
        )}
            ...
            {submissions.map((s) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="text-xs text-gray-500">{getAnswerDetails(s)}</p>
                  <button
                    onClick={() => alert(`Respuestas JSON: ${s.answersContent}`)}
